...
func TodoTasksFromView(tasks []TodoTaskView) []tool.TodoTask {
	out := make([]tool.TodoTask, 0, len(tasks))
	for _, task := range tasks {
		out = append(out, tool.TodoTask{
			ID:              task.ID,
...
func TodoTaskViewsFromTasks(tasks []tool.TodoTask) []TodoTaskView {
	out := make([]TodoTaskView, 0, len(tasks))
	for _, task := range tasks {
		out = append(out, TodoTaskView{
			ID:              task.ID,
			Subject:         task.Subject,
			Description:     task.Description,
			ActiveForm:      task.ActiveForm,
