...
func TestProjectTasksAndWorktreePatches(t *testing.T) {
	...
	task := tool.TodoTask{
		ID:              "1",
		Subject:         "Refactor",
		Status:          tool.TodoStatusInProgress,
		CreatedAt:       taskTime,
		UpdatedAt:       taskTime,
	...
	transcript, err := Project([]Record{
		{TranscriptID: "tx-1", Time: time.Now(), Type: RecordStarted},
		{TranscriptID: "tx-1", Time: time.Now(), Type: RecordStatePatched, State: &StateRecord{Ops: []PatchOp{PatchTasks([]tool.TodoTask{task}), PatchWorktree(wt)}}},
	}, nil)
	if err != nil {
		t.Fatalf("Project(): %v", err)
	}
