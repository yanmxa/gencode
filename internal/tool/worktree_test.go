...
func TestEnterWorktreePrepareInteractionUsesNameParam(t *testing.T) {
	...
	request, err := tool.PrepareInteraction(context.Background(), map[string]any{"name": "feature-slug"}, "/repo")
	if err != nil {
		t.Fatalf("PrepareInteraction() error: %v", err)
	}

	req, ok := request.(*EnterWorktreeRequest)
	if !ok {
		t.Fatalf("expected *EnterWorktreeRequest, got %T", request)
	}
...
func TestEnterWorktreeExecuteWithResponse(t *testing.T) {
	...
	t.Run("declined", func(t *testing.T) {
		result := tool.ExecuteWithResponse(context.Background(), nil, &EnterWorktreeResponse{Approved: false}, "/repo")
		if !result.Success {
	...
	t.Run("approved", func(t *testing.T) {
		result := tool.ExecuteWithResponse(context.Background(), nil, &EnterWorktreeResponse{
			Approved:     true,
...
func TestExitWorktreePrepareInteraction(t *testing.T) {
	...
	t.Run("defaults to remove", func(t *testing.T) {
		request, err := tool.PrepareInteraction(context.Background(), nil, "/repo")
		if err != nil {
			t.Fatalf("PrepareInteraction() error: %v", err)
		}

		req, ok := request.(*ExitWorktreeRequest)
		if !ok {
			t.Fatalf("expected *ExitWorktreeRequest, got %T", request)
		}
	...
	t.Run("parses action and discard flag", func(t *testing.T) {
		request, err := tool.PrepareInteraction(context.Background(), map[string]any{
			"action":          "keep",
		...
		req := request.(*ExitWorktreeRequest)
	...
	t.Run("rejects invalid action", func(t *testing.T) {
		_, err := tool.PrepareInteraction(context.Background(), map[string]any{"action": "archive"}, "/repo")
		if err == nil {
...
func TestExitWorktreeExecuteWithResponse(t *testing.T) {
	...
	t.Run("declined", func(t *testing.T) {
		result := tool.ExecuteWithResponse(context.Background(), nil, &ExitWorktreeResponse{Approved: false}, "/repo")
		if !result.Success {
	...
	t.Run("approved", func(t *testing.T) {
		result := tool.ExecuteWithResponse(context.Background(), nil, &ExitWorktreeResponse{
			Approved:     true,
			RestoredPath: "/repo",
		}, "/repo")
		if !result.Success {
