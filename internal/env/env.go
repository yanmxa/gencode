...
//
//	Pair("PROJECT_DIR", "/tmp") → ["GEN_PROJECT_DIR=/tmp", "CLAUDE_PROJECT_DIR=/tmp"]
...
func Pairs(kvs ...string) []string {
	if len(kvs)%2 != 0 {
		panic("env.Pairs: odd number of arguments")
	}
	out := make([]string, 0, len(kvs))
	for i := 0; i < len(kvs); i += 2 {
		out = append(out, Pair(kvs[i], kvs[i+1])...)
	}
	return out
}
...
//
//	PairF("PLUGIN_ROOT_%s", "CODEX", "/path") →
//	  ["GEN_PLUGIN_ROOT_CODEX=/path", "CLAUDE_PLUGIN_ROOT_CODEX=/path"]
func PairF(keyFmt, keyArg, value string) []string {
