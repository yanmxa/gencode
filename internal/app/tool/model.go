...
func (s *Model) EnterSelect(width, height int, disabledTools map[string]bool, mcpTools func() []provider.Tool) error {
...
func (s *Model) updateFilter() {
	if s.searchQuery == "" {
		...
		for _, t := range s.tools {
			...
			if shared.FuzzyMatch(strings.ToLower(t.Name), query) ||
				shared.FuzzyMatch(strings.ToLower(t.Description), query) {
...
func (s *Model) Render() string {
	...
	if len(s.filteredTools) == 0 {
		sb.WriteString(shared.SelectorHintStyle.Render("  No tools match the filter"))
		sb.WriteString("\n")
	} else {
