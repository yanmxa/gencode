...
func (s *Model) updateFilter() {
	if s.searchQuery == "" {
		...
		for _, sk := range s.skills {
			...
			if shared.FuzzyMatch(strings.ToLower(sk.FullName()), query) ||
				shared.FuzzyMatch(strings.ToLower(sk.Name), query) ||
				shared.FuzzyMatch(strings.ToLower(sk.Description), query) {
...
func (s *Model) Render() string {
	...
	if len(s.filteredSkills) == 0 {
		sb.WriteString(shared.SelectorHintStyle.Render("  No skills match the filter"))
		sb.WriteString("\n")
	} else {
	...
	sb.WriteString("\n")
	sb.WriteString(shared.SelectorHintStyle.Render("\u2191/\u2193 navigate \u00b7 Tab level \u00b7 Enter toggle \u00b7 Esc close"))
