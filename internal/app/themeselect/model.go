// Package themeselect provides a one-time TUI for choosing the color theme.
package themeselect
...
// SelectedMsg is emitted when the user confirms a theme choice.
...
var (
	...
	selectorHintStyle   = lipgloss.NewStyle().Foreground(theme.CurrentTheme.TextDim).MarginTop(1)
)
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case "enter", " ":
		return m, func() tea.Msg { return SelectedMsg{Theme: choices[m.cursor].value} }
	case "ctrl+c", "q":
		return m, tea.Quit
	}
	return m, nil
...
// capture wraps model to intercept SelectedMsg and quit the program.
type capture struct {
	inner    model
	selected string
}
...
func Run() (string, error) {
	p := tea.NewProgram(capture{inner: New()})
	final, err := p.Run()
	if err != nil {
		return "", err
	}
