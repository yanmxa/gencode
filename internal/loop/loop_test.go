...
func newTestLoop(mp provider.LLMProvider) *Loop {
	c := provider.NewLLM(mp, "test-model", 8192)
	return &Loop{
...
func TestAddResponse(t *testing.T) {
	loop := &Loop{Client: provider.NewLLM(&mockProvider{}, "test", 0)}
...
func TestLoopClientAccess(t *testing.T) {
	c := provider.NewLLM(&mockProvider{}, "model-a", 0)
	loop := &Loop{Client: c}
	...
	c2 := provider.NewLLM(&mockProvider{}, "model-b", 0)
	loop.Client = c2
