// Keyboard input handling: key dispatch, submit, history navigation, stream cancel,
// image paste, window resize, skill invocation.
package tui
...
func (m *model) handleKeypress(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	// Delegate to active modal components first
...
func (m *model) delegateToActiveModal(msg tea.KeyMsg) (bool, tea.Cmd) {
	...
	if m.planPrompt != nil && m.planPrompt.IsActive() {
		return true, m.planPrompt.HandleKeypress(msg)
	}
	if m.questionPrompt.IsActive() {
		return true, m.questionPrompt.HandleKeypress(msg)
	}
	if m.permissionPrompt.IsActive() {
		return true, m.permissionPrompt.HandleKeypress(msg)
	}
	if m.enterPlanPrompt.IsActive() {
		return true, m.enterPlanPrompt.HandleKeypress(msg)
	}

	// Check selectors
	if m.selector.IsActive() {
		return true, m.selector.HandleKeypress(msg)
	}
	if m.toolSelector.IsActive() {
		return true, m.toolSelector.HandleKeypress(msg)
	}
	if m.skillSelector.IsActive() {
		return true, m.skillSelector.HandleKeypress(msg)
	}
	if m.agentSelector.IsActive() {
		return true, m.agentSelector.HandleKeypress(msg)
	}
	if m.mcpSelector.IsActive() {
		return true, m.mcpSelector.HandleKeypress(msg)
	}
	if m.pluginSelector.IsActive() {
		return true, m.pluginSelector.HandleKeypress(msg)
	}
	if m.sessionSelector.IsActive() {
		return true, m.sessionSelector.HandleKeypress(msg)
	}
	if m.memorySelector.IsActive() {
		return true, m.memorySelector.HandleKeypress(msg)
	}
...
func (m *model) handleSubmit() (tea.Model, tea.Cmd) {
	...
	if input != "" {
		...
		m.historyIndex = -1
		m.tempInput = ""
		history.Save(m.cwd, m.inputHistory)
	}
...
func (m *model) handleWindowResize(msg tea.WindowSizeMsg) (tea.Model, tea.Cmd) {
	...
	if m.planPrompt != nil {
		m.planPrompt.SetSize(msg.Width, msg.Height)
	}
